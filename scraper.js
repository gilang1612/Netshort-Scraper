...
// =====================================================================
// 3. FUNGSI UTAMA - API REQUEST (INTERNAL)
// =====================================================================
async function sendAppRequest(url, payload) {
    try {
        ...
        const response = await axios.post(url, encryptedBody, { headers: headers });
...
async function scrapeDashboard() {
    console.log('\n[+] Scraping Dashboard...');
    const result = await sendAppRequest(ENDPOINTS.dashboard.url, ENDPOINTS.dashboard.payload);
    console.log('[✓] Dashboard:', JSON.stringify(result, null, 2));
    return result;
}
 ...
 */
async function scrapeDetailInfo(params = {}) {
    console.log('\n[+] Scraping Detail Info...');
    const payload = ENDPOINTS.detail_info.payload(params);
    console.log(`    Params: codec=${payload.codec}, shortPlayId=${payload.shortPlayId}, playClarity=${payload.playClarity}`);
    const result = await sendAppRequest(ENDPOINTS.detail_info.url, payload);
    console.log('[✓] Detail Info:', JSON.stringify(result, null, 2));
    return result;
}
...
async function scrapeSearch(params = {}) {
    ...
    console.log(`    Params: searchCode=${payload.searchCode}, pageNo=${payload.pageNo}, pageSize=${payload.pageSize}`);
    const result = await sendAppRequest(ENDPOINTS.search.url, payload);
...
async function scrapeAll(options = {}) {
    ...
    if (!options.skipDashboard) {
        results.dashboard = await scrapeDashboard();
    }

    if (!options.skipDetail) {
        results.detail_info = await scrapeDetailInfo(options.detailParams);
    }

    if (!options.skipSearch) {
        results.search = await scrapeSearch(options.searchParams);
    }
...
/**
 * Set custom authorization token
 * @param {string} token - New bearer token
 */
function setAuthToken(token) {
    // Update headers in sendAppRequest would require refactoring
    // For now, this is a placeholder for future implementation
    console.log('[!] setAuthToken: Feature placeholder - edit scraper.js directly for now');
}

/**
 * Set custom device code
 * @param {string} deviceCode - New device code
 */
function setDeviceCode(deviceCode) {
    console.log('[!] setDeviceCode: Feature placeholder - edit scraper.js directly for now');
}
...
async function main() {
    ...
    if (!command || command === 'all') {
        ...
        await scrapeDetailInfo({ shortPlayId });
    } else if (command === 'search') {
        const keyword = args[1] || 'ceo';
        const page = args[2] || '1';
        await scrapeSearch({ searchCode: keyword, pageNo: page });
    } else {
        console.log(`
